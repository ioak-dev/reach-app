...
const OakRouteGraph = (props: Props) => {
  ...
  const authenticate = async (type: string, redirect = true) => {
    ...
    const accessToken = props.cookies.get(`reach-access_token`);
    const refreshToken = props.cookies.get(`reach-refresh_token`);
    if (accessToken && refreshToken) {
      ...
      if (data?.authorizeUser) {
        ...
        if (data.authorizeUser.accessToken) {
          ...
          props.cookies.set(`reach-access_token`, newAccessToken);
        }
        dispatch(
          addAuth({
        ...
        props.cookies.remove(`reach-access_token`);
        props.cookies.remove(`reach-refresh_token`);
